...
//{{{KERNELS
__global__ void
d_initTimeline( REAL* d_timeline, unsigned int numT, const REAL t){
...
__global__ void
d_initNUM( REAL* d_num, unsigned int num_size, const REAL d, REAL myIndex, const REAL s){
    const unsigned long gid = blockIdx.x*blockDim.x + threadIdx.x;
    if(gid < num_size) {
...
void initGrid_GPU(  const REAL s0, const REAL alpha, const REAL nu,const REAL t, const unsigned numX, const unsigned numY, const unsigned numT, REAL* d_myX, REAL* d_myY, REAL* d_myTimeline, REAL& myXindex, REAL& myYindex) {
...
//    const unsigned int result_size = numX*numY*outer*sizeof(REAL);
    ...
    cudaMalloc((REAL**)&d_result, numX*numY*outer*sizeof(REAL)); 
    cudaMalloc((REAL**)&d_varX, numX*numY*sizeof(REAL)); //XY
    cudaMalloc((REAL**)&d_varY, numX*numY*sizeof(REAL)); //XY
    cudaMalloc((REAL**)&d_x, numX*sizeof(REAL)); 
    cudaMalloc((REAL**)&d_y, numY*sizeof(REAL));
    cudaMalloc((REAL**)&d_timeline, numT*sizeof(REAL));
    cudaMalloc((REAL**)&d_dxx, numX*4*sizeof(REAL));
    cudaMalloc((REAL**)&d_dyy, numY*4*sizeof(REAL));
    ...
    cudaMalloc((REAL**)&d_u , outer*numY*numX*sizeof(REAL)); //OYX
    cudaMalloc((REAL**)&d_v , outer*numX*numY*sizeof(REAL)); //OXY
    ...
    initOperator_GPU( d_y, numY, d_dyy);
        ...
        std::vector<std::vector<REAL> > myVarX, myVarY;
        myResult.resize(outer);

        myVarX.resize(numX);
        myVarY.resize(numX);

        REAL *h_result, *h_varX, *h_varY, *h_x, *h_y, *h_timeline, *h_dxx, *h_dyy;
        ...
        //cudaMemcpy( h_result    , d_x       , numX*numY*outer*sizeof(REAL), cudaMemcpyDeviceToHost);
        //cudaMemcpy( h_varX      , d_y       , numX*numY*sizeof(REAL)      , cudaMemcpyDeviceToHost);
        //cudaMemcpy( h_varY      , d_timeline, numX*numY*sizeof(REAL)      , cudaMemcpyDeviceToHost);
...
#pragma omp parallel for default(shared) schedule(static)
        ...
        // array expansion on a b c y yy u v
...
#pragma omp parallel for default(shared) schedule(static)  //Kernel-1: 3D
        for( unsigned k = 0; k < outer; ++ k ) {  // outmost loop
            ...
            {
                //REAL payoff = max(globs.myX[i]-strike, (REAL)0.0); 
                // move this inside the loop to do privatization
                for(unsigned j=0;j<globs.myY.size();++j)
...
// Expand rollback function body to make perfectly nested loops:
