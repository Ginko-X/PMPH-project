...
/*
__global__ void
d_implicit_x_tridag(REAL* u, R)(REAL* d_varX, REAL* d_varY, REAL* d_x, REAL* d_y, REAL* d_timeline, 
    unsigned int g, REAL alpha, REAL beta, REAL nu, 
    unsigned int numX, unsigned int numY){
...
//GPU updateParams
    ...
    *h_u, *h_v;
    ...
    for(int iter = 0; iter < outer*numZ*numZ; iter ++){
        unsigned k = iter /numZ/numZ;
        unsigned j = iter/ numZ;
        unsigned i = iter % numZ;
        if(i< numX && j < numY){
